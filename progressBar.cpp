...
HRESULT progressBar::init(const char* topImage, const char* bottomImage, int x, int y)
{
	_topImage = topImage;
	...
	_progressBarTop = IMAGEMANAGER->findImage(_topImage);
	_progressBarBottom = IMAGEMANAGER->findImage(_bottomImage);

	_width = _progressBarBottom->getWidth();

	_rcProgress = RectMake(_x, _y, _width, _progressBarBottom->getHeight());
